...
void	test_token_list_add_one(void)
{
	...
	assert_int_eq("Token list add one, created the first node", token_list->token->type, T_GREAT);
	assert_true("Token list add one, no next note", token_list->next == NULL);
	token_list_clear(&token_list);
	assert_true("Token list add one, clears list", token_list == NULL);
}
