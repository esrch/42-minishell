...
#define TEST_TOKEN_STR(token, token_str, token_type) \
	...
	assert_eq_str("Token tostring, " #token_type, #token_type, token_str);
...
void	test_token_init_cleanup(void)
{
	...
	assert_eq_int("Token type set", token.type, T_WORD);
	assert_eq_str("Lexeme set", "lexeme", token.lexeme);
	assert_eq_str("Value set", "value", token.value);
	token_cleanup(&token);
}
...
void	test_token_construct_destruct(void)
{
	...
	assert_eq_int("Token construct, token type", token->type, T_WORD);
	assert_eq_str("Token construct, lexeme", "lexeme", token->lexeme);
	assert_eq_str("Token construct, value", "value", token->value);
...
void	test_token_word(void)
{
	...
	assert_eq_int("Token word, token type", token->type, T_WORD);
	assert_eq_str("Token word, lexeme", "lexeme", token->lexeme);
	assert_eq_str("Token word, value", "value", token->value);
	token_destruct(token);
}
...
void	test_token_op(void)
{
	...
	assert_eq_int("Token op, token type", token->type, T_GREAT);
	assert_eq_true("Token op, lexeme", token->lexeme == NULL);
	assert_eq_true("Token op, value", token->value == NULL);
	token_destruct(token);
}
